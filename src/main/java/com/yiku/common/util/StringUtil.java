...
//    }
    ...
    /**
     * @param times （一天要执行的次数，大于1小于24）
     * @return
     */
    public static Long getNextSleepTime(int times) {
        Date date = new Date();
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        Long nowSecends = calendar.getTimeInMillis();
        SimpleDateFormat sdf = new SimpleDateFormat("HH");
        Integer hoursStr = Integer.valueOf(sdf.format(date));
        int split = hoursStr * times / 24;
        calendar.add(Calendar.HOUR_OF_DAY, (split + 1) * (24 / times) - hoursStr);
        Long timesStart = calendar.getTimeInMillis();
        return timesStart - nowSecends;
